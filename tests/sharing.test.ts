...
describe("knowledge_list_access", () => {
  ...
  it("returns all HAS_ACCESS entries with user_id, role, granted_at", async () => {
    ...
    const sharing = parseToolSuccess(body).sharing as Array<
      Record<string, unknown>
    >;

    expect(sharing).toHaveLength(2);
    expect(
      sharing.some((s) => s.user_id === viewer && s.role === "viewer"),
    ).toBe(true);
    expect(
      sharing.some((s) => s.user_id === editor && s.role === "editor"),
    ).toBe(true);
...
describe("full sharing workflow", () => {
  it("share → list_access → access as viewer → revoke → access denied", async () => {
    ...
    const sharing = parseToolSuccess(lb).sharing as Array<
      Record<string, unknown>
    >;
...
describe("knowledge_list_access with limit", () => {
  it("respects limit and returns most recently granted first", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await neo4jClient.shareResource(
        entryId,
        `sharing-limit-viewer-${i}-${Date.now()}`,
        "viewer",
      );
    ...
    const allSharing = parseResult(allBody).sharing as Array<
      Record<string, unknown>
    >;
    ...
    const limited = parseResult(limitBody).sharing as Array<
      Record<string, unknown>
    >;
