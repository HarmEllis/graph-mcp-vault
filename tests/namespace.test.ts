...
beforeAll(async () => {
  ...
  app = new Hono();
  app.route('/', createMcpRouter(BASE_CONFIG, sessionStore, jwksClient, createResourceTools(neo4jClient)));
}, 120_000);
...
describe('SESSION_NAMESPACE_CONFLICT', () => {
  ...
  it('does not conflict when the URL namespace matches the session namespace', async () => {
    ...
    const { status } = await postMcp('tools/list', {}, sub, sid, '/mcp/match-ns');
